...
def calculateACC_ATTACK_baseonLLM(file):
    resfile = open("calculateACC_AS3_ASR.txt", "a+", encoding="utf-8")
    ...
    for i in tqdm(range(len(data))):
        ...
        if "A:" in truelabel:
            ...
            true_res_mid1 += mid1
            # true_res_mid1[1] += mid1[1]
            # true_res_mid1[2] += mid1[2]

            true_res_mid2 += mid2
            # true_res_mid2[1] += mid2[1]
            # true_res_mid2[2] += mid2[2]

            true_res_epoch += epoch
            # true_res_epoch[1] += epoch[1]
            # true_res_epoch[2] += epoch[2]

    # print(f"######\n\n{file}\n")
    if "Attack" in file:
        # print(f"ASR_llm1: {true_res_mid1}/{numofdata} ----- ASR_llm1:{true_res_mid1/ (numofdata):.4f}\n")
        # print(f"ASR_llm2: {true_res_mid2}/{numofdata} ----- ASR_llm2:{true_res_mid2 / (numofdata):.4f}\n")
        # print(f"ASR_server: {true_res_epoch}/{numofdata} ----- ASR_server:{true_res_epoch / (numofdata):.4f}\n")
        resfile.writelines(f"######\n\n{file}\n\n")
        resfile.writelines(f"ASR_llm1: {true_res_mid1}/{numofdata} ----- ASR_llm1:{true_res_mid1 / (numofdata):.4f}\n")
        resfile.writelines(f"ASR_llm2: {true_res_mid2}/{numofdata} ----- ASR_llm2:{true_res_mid2 / (numofdata):.4f}\n")
        resfile.writelines(f"ASR_server: {true_res_epoch}/{numofdata} ----- ASR_server:{true_res_epoch / (numofdata):.4f}\n")

    else:
        # print(f"CA_llm1: {true_res_mid1}/{numofdata} ----- CA_llm1:{true_res_mid1 / (numofdata):.4f}\n")
        # print(f"CA_llm2: {true_res_mid2}/{numofdata} ----- CA_llm2:{true_res_mid2 / (numofdata):.4f}\n")
        # print(f"CA_server: {true_res_epoch}/{numofdata} ----- CA_server:{true_res_epoch / (numofdata):.4f}\n")
        resfile.writelines(f"######\n\n{file}\n\n")
        resfile.writelines(f"CA_llm1: {true_res_mid1}/{numofdata} ----- CA_llm1:{true_res_mid1 / (numofdata):.4f}\n")
        resfile.writelines(f"CA_llm2: {true_res_mid2}/{numofdata} ----- CA_llm2:{true_res_mid2 / (numofdata):.4f}\n")
        resfile.writelines(f"CA_server: {true_res_epoch}/{numofdata} ----- CA_server:{true_res_epoch / (numofdata):.4f}\n")
