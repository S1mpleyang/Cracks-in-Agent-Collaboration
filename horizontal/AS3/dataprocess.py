...
def calculateACC_baseonLLM(file, output_path):
    ...
    for i in tqdm(range(len(data))):
        ...
        mid10 = one["mid_llm1"][0]
        mid20 = one["mid_llm2"][0]
        epoch0 = one["epoch 0-3 res"][0]
        curinfo = [mid10, mid20, epoch0]
        ...
        result.append(
            {"ID": one["ID"], "TrueLabel": one["TrueLabel"], "mid_tool1":curinfo[0], "mid_tool2":curinfo[1], "epoch 0-3 res": curinfo[2]}
        )
...
def calculateACC_ATTACK_baseonLLM(file, output_path):
    ...
    for i in tqdm(range(len(data))):
        ...
        result.append(
            {"ID": one["ID"], "TrueLabel": one["TrueLabel"], "mid_tool1": curinfo[0], "mid_tool2": curinfo[1],
             "epoch 0-3 res": curinfo[2]}
        )
