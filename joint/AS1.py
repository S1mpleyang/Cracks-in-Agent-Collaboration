...
def run_using_dataset(file="qa-data", llm1="llama3", llm2="llama3", server="llama3", summaryflag="direct", start_round=0, end_round=400):
    ...
    for i in tqdm(range(start_round, round_)):
        ...
        with torch.no_grad():
            for j in range(3):
                ...
                server_query = user_que
                classifier_res = SERVER.response(prompt=generate_malicious_distribute_prompt(server_query), max_new_tokens=20)
                mid_res.append(classifier_res)

            # knowledge process for next round
            # if summaryflag == "direct":
            #     user_que = server_query
                # user_que = "\n".join([user_que, response1, attack])
            # elif summaryflag == "vote":
            #     attack_ = server.response(prompt=user_que) + attack  # edition1 效果不好
