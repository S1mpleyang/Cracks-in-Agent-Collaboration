...
def calculateACC(file):
    ...
    DASR0, DASR1, DASR2 = 0, 0, 0

    CR0, CR1, CR2 = 0, 0, 0
    DCR0, DCR1, DCR2 = 0, 0, 0
    ...
    resfile.writelines(f"DASR0: {DASR0}/{len(data)} ----- targetASR0:{DASR0 / (len(data)):.4f}\n")
    resfile.writelines(f"DASR1: {DASR1}/{len(data)} ----- targetASR1:{DASR1 / (len(data)):.4f}\n")
    resfile.writelines(f"DASR2: {DASR2}/{len(data)} ----- targetASR2:{DASR2 / (len(data)):.4f}\n")
    ...
    resfile.writelines(f"DCR0: {DCR0}/{len(data)} ----- DCR0:{DCR0 / (len(data)):.4f}\n")
    resfile.writelines(f"DCR1: {DCR1}/{len(data)} ----- DCR1:{DCR1 / (len(data)):.4f}\n")
    resfile.writelines(f"DCR2: {DCR2}/{len(data)} ----- DCR2:{DCR2 / (len(data)):.4f}\n")
